...
{
    pGetColor = getcolor;
    std::string shader_folder = "/home/zed/Project/libGL_UI/libGL_ContentBase/Shaders";
    ...
    mShader = std::make_unique<GLUtils::Shader>(vsShader, fsShader, gsShader);
    mShader->use();
...
{
    if(!bInited)
        std::runtime_error("NodeDrawer: Draw was called before Init!");
    mShader->use();
    mShader->set("diffuse_strength",mDiffuseStrength);
...
void SurfelDrawer::UpdateBuffer(size_t newSize, bool force)
{
    ...
    mBufferSize = (std::floor(newSize/1e6) + 1) * 1e6; // std::floor向下取整
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *) (mBufferSize * (3) * sizeof(GLfloat)));
    
    // vec3 normal
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *) (mBufferSize * (3+3) * sizeof(GLfloat)));

    glEnableVertexAttribArray(3);
    glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 1 * sizeof(GLfloat), (void *) (mBufferSize * (3+3+3) * sizeof(GLfloat)));
...
void SurfelDrawer::Update( const std::vector< std::shared_ptr<inseg_lib::Surfel> >& surfels )
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    // position
    glBufferSubData(GL_ARRAY_BUFFER, 0, mPointSize * 3 * sizeof(GLfloat), points.data());
    
    // color
    glBufferSubData(GL_ARRAY_BUFFER, mBufferSize * (3) * sizeof(GLfloat), mPointSize * 3 * sizeof(GLfloat), colors.data());
    
    // normal
    glBufferSubData(GL_ARRAY_BUFFER, mBufferSize * (3+3) * sizeof(GLfloat), mPointSize * 3 * sizeof(GLfloat), normals.data());
    glBufferSubData(GL_ARRAY_BUFFER, mBufferSize * (3+3+3) * sizeof(GLfloat), mPointSize * 1 * sizeof(GLfloat), radius.data());
