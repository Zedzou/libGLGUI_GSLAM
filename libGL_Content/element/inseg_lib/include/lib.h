...
namespace inseg_lib 
{
    class InSegLibInterface 
    {
        public:
            ...
            virtual void ProcessFrame(const cv::Mat& depth_map, const cv::Mat& color_map) = 0;

            // Initialize camera intrinsics for image pyramid.
            virtual void InitializeCamera(const Eigen::Matrix3d& intrinsics, const int image_width, const int image_height) = 0;

            // Returns the current camera pose in world coordinates.
            virtual const Eigen::Matrix4f& pose() const = 0;

            // Set pose from outside (e.g. tango). 纯虚函数，只能声明不能调用，留待派生类中声明（=0）
    ...
    {
        public:
            InSegLib(std::shared_ptr<inseg_lib::SegmentsInterface> segments, InSegConfig& config, MainConfig& main_config, MapConfig& map_config, const SegmentationConfig& segmentation_config);
            
            // Main process.
            void ProcessFrame(const cv::Mat& depth_map, const cv::Mat& color_map) override;

            // Initialize map data and camera.
            void InitializeMap(const Eigen::Matrix4f& pose) override;
            void InitializeCamera(const Eigen::Matrix3d& intrinsics, const int image_width, const int image_height) override;
            void Reset() override;
            ...
            void set_pose(const Eigen::Matrix4f& pose) override; // 重写基类的虚函数（override）
