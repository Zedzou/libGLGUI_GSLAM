...
namespace inseg_lib 
{
  ...
  {
    public:
      virtual int Segment(const cv::Mat& point_cloud, const cv::Mat& normal_map, cv::Mat* label_map) = 0;
  };
  ...
  class InSegSegmentation : public SegmentationInterface 
  {
    public:
      InSegSegmentation(const float depth_edge_threshold,
                        ...
                        const int min_region_size):depth_edge_threshold_(depth_edge_threshold),
                                                   uncertainty_coefficient_depth_(uncertainty_coefficient_depth),
                                                   min_region_size_(min_region_size) {}
      int Segment(const cv::Mat& point_cloud, const cv::Mat& normal_map, cv::Mat* label_map) override;

    private:
      void Segment(cv::Mat* label_map);
      int Relabel(cv::Mat* label_map);
      void CalculateGeometricalEdge(const cv::Mat& point_cloud, const cv::Mat& normal_map);
      void RegionGrowing(const cv::Mat& point_cloud, const cv::Mat& normal_map, cv::Mat* label_map);
      void CreateEdges(const cv::Mat& normal_edge, const cv::Mat& point_cloud, const cv::Mat& normal_map);
