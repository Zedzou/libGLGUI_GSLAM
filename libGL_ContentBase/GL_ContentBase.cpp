#include "GL_ContentBase.h"
using namespace GLRenderer;

GL_ContentBase::GL_ContentBase(const std::string &name, int width, int height,bool visible)
{
    // 创建窗口，关联ImgUI和窗口
    GL_base::initWindow(name, width, height, visible);
    ...
    // 相机视角控制
    camPose = glm::vec3(0.0f, 0.0f,  -4.5f);
...
// 键盘按键注册
void GL_ContentBase::basicInputRegistration()
{
    // 按键C显示网格
...
// 画UI(virtual override)
void GL_ContentBase::drawUI() 
{
    GL_base::drawUI(); // UI设计
    cameraUI();
    glCam->drawUI();
...
// OpenGL渲染(virtual override)
void GL_ContentBase::drawGL()
{
    processInput(window_->window);
}

// OpenGL渲染窗口(虚函数)
void GL_ContentBase::processInput(GLFWwindow* window)
{
    auto io = ImGui::GetIO();
...
// 鼠标控制
bool GL_ContentBase::mouseControl()
{
...
// 显示注册的键盘函数
void GL_ContentBase::showRegisteredKeyFunction()
{
...
// 显示相机UI
void GL_ContentBase::cameraUI() 
{
    if(!bShowCameraUI) return;
}

// 基础处理
void GL_ContentBase::basicProcess() 
{
    ...
    if (bShowGrid) 
    {
        ...
        GLUtils::Mesh *Plane = (GLUtils::Mesh *) glObjests["Plane"];
        GLUtils::Shader *shader = glShaders["grid"];
    ...
    {
        glDisable(GL_DEPTH_TEST);
        RenderText("FPS: " + std::to_string((int) std::floor(fps_->getFPS())), 10.f, 10.f, 0.5f, glm::vec3(0.5f, 0.8f, 0.2f));
        glEnable(GL_DEPTH_TEST);
    }
...
// 画轨迹
void GL_ContentBase::plot_trajectory(const glm::mat4 *projection)
{
    const std::string name = "Trajectory";
    if(glShaders.find(name) == glShaders.end())return;
    GLUtils::Shader *shader = glShaders[name];
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, trajectories_.size() * 3 * sizeof(GLfloat), trajectories_.data());
    glDrawArrays(GL_LINE_STRIP, 0, trajectories_.size());
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}

// 添加轨迹
void GL_ContentBase::add_trajectory(float x, float y, float z, float interval)
{
...
// 渲染文本
void GL_ContentBase::RenderText(const std::string &text, float pos_x, float pos_y, float scale, const glm::vec3 &color) 
{
    RenderText(glVertexArrays["textVAO"], glBuffers["textVBO"], glShaders["Text"], text, pos_x,pos_y,scale,color);
}

void GL_ContentBase::RenderText(GLuint VAO, GLuint VBO, GLUtils::Shader *shader, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) 
{
    shader->use();
...
// -------------------着色器编译-------------------
void GL_ContentBase::buildText()
{
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
        glShaders["Text"] = new GLUtils::Shader(shaderPath+ "text.vs", shaderPath + "text.fs");
        glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(window_->runtimeWidth), 0.0f, static_cast<GLfloat>(window_->runtimeHeight));
        glShaders["Text"]->use();
...
void GL_ContentBase::buildTrajectory() 
{
    // Trajectory
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
        ...
        glShaders[name] = new GLUtils::Shader(shaderPath + "camera_shader.vs", shaderPath + "camera_shader.fs");
        glShaders[name]->use();
...
void GL_ContentBase::buildFreeType()
{
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
...
void GL_ContentBase::buildCamera()
{
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    // Camera
    {
        ...
        glShaders["Camera"] = new GLUtils::Shader(shaderPath + "camera_shader.vs",
...
void GL_ContentBase::buildScreen()
{
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    // Screen
    {
        glShaders["Screen"] = new GLUtils::Shader(shaderPath + "2D.vs", shaderPath + "screen.fs");
        glm::mat4 projection = glm::ortho(0.0f, 
                                          static_cast<GLfloat>(window_->runtimeWidth), 
                                          0.0f,
        ...
        //TODO: debug screen3D
        glShaders["Screen3D"] = new GLUtils::Shader(shaderPath + "2D_in_3D.vs",shaderPath + "2D_in_3D.fs");
        glShaders["Screen3D"]->use();
        glShaders["Screen3D"]->set("screenTexture", 0);
    }
}

void GL_ContentBase::buildGrid()
{
    // Grid
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
        glShaders["grid"] = new GLUtils::Shader(shaderPath + "grid.vs",shaderPath + "grid.fs");
        glObjests["Plane"] = (GLUtils::Model_base *) new GLUtils::Mesh( GLUtils::ShapeVertices::plane );
    }
}