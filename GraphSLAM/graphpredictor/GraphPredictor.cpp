...
GraphPredictor::GraphPredictor(const ConfigPSLAM *configPslam) : mConfigPslam(configPslam), mUpdated(false)
{
    ...
    mbThread = configPslam->use_thread; // 是否使用线程
...
void find_nodes_has_same_part(const std::map<int, NodePtr> &nodes, unsigned int s_idx, int t_idx, std::set<int> &has, std::set<int> &exclude)
{
...
static MemoryBlock2D ConcatObjFeature(size_t n_node, size_t dim_obj_feature, Ort::Value &obj_feature, const MemoryBlock2D &descriptor) {
    ...
    MemoryBlock2D tmp_out_enc_obj(obj_feature.GetTensorMutableData<float>(), MemoryBlock::FLOAT, {static_cast<unsigned long>(n_node), dim_obj_feature});
...
void GraphPredictor::UpdatePrediction(const std::map<int,NodePtr> &vNodes, const std::map<int,std::pair<size_t,size_t>> &sizeAndEdge) 
{
