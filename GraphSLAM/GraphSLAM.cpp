...
std::vector<std::shared_ptr<inseg_lib::Surfel>> GraphSLAM::FilterSegment( int segment_filter,  const std::vector<std::shared_ptr<inseg_lib::Surfel>> &surfels)
{
...
// 保存为PLY点云
void GraphSLAM::SaveSurfelsToPLY(const std::string &output_folder, const std::string &output_name, const std::vector<std::shared_ptr<inseg_lib::Surfel>> &surfels, bool binary) 
{
...
json11::Json GraphSLAM::GetSceneGraph(bool full)
{
    ...
    else 
    {
        ...
        {
            if (edge.second->GetLabel() == Edge::Same())
                if (mGraph->nodes.at(edge.second->nodeFrom)->GetLabel() != mGraph->nodes.at(edge.second->nodeTo)->GetLabel())
                    continue;
            std::string name = std::to_string(edge.second->nodeFrom) + "_" + std::to_string(edge.second->nodeTo);
            json11::Json::object probs;
