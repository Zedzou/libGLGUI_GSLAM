...
// 构造函数
Graph::Graph(const ConfigGSLAM *configGSLAM, bool useThread) : mConfigGSLAM(configGSLAM), mbThread(useThread)
{
    if(mbThread){
        // std::thread::hardware_concurrency() 获取CPU核心数
...
// 更新
void Graph::Update(const SurfelPtr surfel, const Eigen::Vector3f& pos, const Eigen::Vector3f& normal) 
{
...
int Graph::UpdateLabel(const SurfelPtr surfel, const int label) 
{
    ...
    try 
    {
        ...
        std::unique_lock<std::mutex> lock2(mMutNode);
        {
            if (edges.find(pair) == edges.end()) 
            {
                ...
                {
                    if (nodes.at(edge->nodeFrom)->GetLabel() != nodes.at(edge->nodeTo)->GetLabel())
                        return -1;
                }
            ...
            else 
            {
                ...
                oldedge->UpdatePrediction(edge->labelProp, mConfigGSLAM->use_fusion);
...
{
    auto edge_ = std::make_shared<GSLAM::Edge>(*edge);
    edge_->nodeFrom=from;
    edge_->nodeTo=to;
...
void Graph::UpdateSelectedNodes(const std::unordered_set<int> &filtered_selected_nodes, const size_t time, const bool force) 
{
    ...
    for (auto node_idx : filtered_selected_nodes) 
    {
        ...
        NodePtr node;
        {
            ...
            if (node->surfels.size() < (size_t)mConfigGSLAM->filter_num_node) continue;
        }
        if(mbThread)
            mPools->runTask( std::bind(&GSLAM::Node::UpdateSelectedNode, node.get(), time, mConfigGSLAM->filter_num_node, mConfigGSLAM->n_pts, force) );
        else
            node->UpdateSelectedNode(time, mConfigGSLAM->filter_num_node, mConfigGSLAM->n_pts, force);
    }
