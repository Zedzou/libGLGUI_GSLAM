...
GUI_GSLAM::GUI_GSLAM(const std::string &name, int width, int height,bool visible)
{
...
// OpenGL渲染窗口(虚函数)
void GUI_GSLAM::processInput(GLFWwindow* window)
{
    auto io = ImGui::GetIO();
...
// 基础输入注册
void GUI_GSLAM::basicInputRegistration()
{
...
// 建立轨迹
void GUI_GSLAM::buildTrajectory() 
{
    // Trajectory
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
...
// 画轨迹
void GUI_GSLAM::plot_trajectory(const glm::mat4 *projection)
{
...
// 添加轨迹
void GUI_GSLAM::add_trajectory(float x, float y, float z, float interval)
{
...
// 渲染文本
void GUI_GSLAM::RenderText(GLuint VAO, GLuint VBO, glUtil::Shader *shader, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) 
{
    shader->use();
...
// 显示注册的键盘函数
void GUI_GSLAM::showRegisteredKeyFunction()
{
...
// 显示相机UI
void GUI_GSLAM::cameraUI() 
{
    if(!bShowCameraUI) return;
}

// 渲染文本
void GUI_GSLAM::RenderText(const std::string &text, float pos_x, float pos_y, float scale, const glm::vec3 &color) 
{
