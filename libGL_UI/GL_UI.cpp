...
GL_UI::GL_UI(const std::string &name, int width, int height,bool visible)
{
    // 创建窗口，关联ImgUI和窗口
...
// 键盘按键注册
void GL_UI::basicInputRegistration()
{
    // 按键C显示网格
...
// OpenGL渲染窗口(虚函数)
void GL_UI::processInput(GLFWwindow* window)
{
    auto io = ImGui::GetIO();
...
// 显示注册的键盘函数
void GL_UI::showRegisteredKeyFunction()
{
...
// 画轨迹
void GL_UI::plot_trajectory(const glm::mat4 *projection)
{
...
// 添加轨迹
void GL_UI::add_trajectory(float x, float y, float z, float interval)
{
...
// 渲染文本
void GL_UI::RenderText(const std::string &text, float pos_x, float pos_y, float scale, const glm::vec3 &color) 
{
    RenderText(glVertexArrays["textVAO"], glBuffers["textVBO"], glShaders["Text"], text, pos_x,pos_y,scale,color);
}

void GL_UI::RenderText(GLuint VAO, GLuint VBO, glUtil::Shader *shader, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) 
{
    shader->use();
...
// -------------------着色器编译-------------------
void GL_UI::buildText()
{
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
        glShaders["Text"] = new glUtil::Shader(shaderPath+ "text.vs", shaderPath + "text.fs");
...
void GL_UI::buildTrajectory() 
{
    ...
    const std::string shaderPath = std::string(GUI_FOLDER_PATH) + "Shaders/";
    {
        ...
        glShaders[name] = new glUtil::Shader(shaderPath + "camera_shader.vs", shaderPath + "camera_shader.fs");
        glShaders[name]->use();
